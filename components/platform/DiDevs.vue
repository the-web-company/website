...
<template>
  <div class="bg-purple-900 rounded-md max-w-5xl p-2">
    <header class="border-b border-purple-700">
      <nav class="flex pb-4">
        <ul role="list" class="flex min-w-full flex-none gap-x-6 font-semibold leading-6 text-xs">
          <li class="flex items-center gap-2 cursor-pointer hover:text-rose-400"><UIcon name="i-heroicons-key" class="h-5 w-5" />API</li>
          <li class="flex items-center gap-2 cursor-pointer hover:text-rose-400"><UIcon name="i-heroicons-document-text" class="h-5 w-5" />Documentation</li>
          <li class="flex items-center gap-2 cursor-pointer text-rose-400"><UIcon name="i-heroicons-sparkles" class="h-5 w-5" />Playground</li>
        </ul>
      </nav>
    </header>

    <div class="grid md:grid-cols-7 md:divide-x md:divide-purple-700 gap-2 mt-2">
      <nav class="hidden md:flex flex-col gap-y-2">
        <ul class="space-y-1">
          <li v-for="(endpoint, index) of endpoints" :key="index">
            <button :class="[endpoint.active ? 'bg-purple-800 ' : '', 'group flex items-center text-xs rounded-md hover:bg-purple-800 p-1.5 w-full']">
      ...
      <div class="md:col-span-6 px-2 space-y-4 md:space-y-8">
        <header class="flex items-center gap-4 w-full">
          <span class="text-purple-400 text-xs px-2 py-1.5 ring-1 ring-inset ring-purple-700 rounded-md w-full flex gap-4 overflow-x-auto">
        ...
        <div class="flex flex-col-reverse md:flex-row gap-4">
          ...
          <div class="md:w-1/2 space-y-4">
            <div class="bg-purple-800 rounded-md">
              <div class="bg-purple-700 flex justify-between items-center p-2 rounded-t-md">
                <h4 class="text-purple-100 text-sm">Request</h4>
                <UButton icon="i-heroicons-clipboard" size="sm" color="primary" square variant="solid" />
              </div>
              <div class="p-2">
                <pre class="text-purple-100 whitespace-pre-wrap text-xs">
curl --request GET \ 
  ...
  --header 'api-key: ba376422-02e3-93f0ec7566d6'
            ...
            <div class="hidden md:block bg-purple-800 rounded-md">
              <div class="bg-purple-700 flex justify-between items-center p-2 rounded-t-md">
                <h4 class="text-purple-100 text-sm py-2">Response</h4>
              </div>
              <div>
                <div class="flex flex-col justify-center items-center gap-2 h-60 text-neutral-100 text-sm"></div>
              </div>
            </div>
          </div>
        </div>
