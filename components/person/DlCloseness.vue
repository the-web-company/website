...
<template>
  <div class="bg-purple-900 ring-1 ring-purple-700 rounded-md py-2 px-2 xl:px-6 2xl:px-8" @mouseover="localState.tab = 1" @mouseleave="localState.tab = 0">
    <div class="flex items-center justify-between w-full h-full">
      ...
      <div
        class="mx-2 z-10 border bg-purple-950 border-purple-700 rounded-md py-1 md:py-2 px-2 md:px-4 relative before:content-[''] before:w-28 sm:before:w-32 md:before:w-44 lg:before:w-32 xl:before:w-52 2xl:before:w-72 3xl:before:w-80 before:border-b before:absolute before:top-1/2 before:border-purple-700 before:right-full"
      >
        ...
        <ul class="h-56 divide-y divide-purple-700" v-if="localState[currentTab].length">
          ...
          <li class="flex p-2 md:py-4" v-for="(item, index) of localState[currentTab]" :key="index">
            ...
            <div class="ml-3 flex flex-col justify-center">
              ...
              <p class="text-sm text-purple-400 hidden sm:block">{{ item.title }}</p>
            </div>
          </li>
        ...
        <div v-else class="h-56 text-purple-400 flex justify-center items-center">
          <p class="xl:w-3/4 text-center text-sm">There are no close connections via {{ currentTab }}</p>
        </div>
      </div>

      <div
        class="text-right relative before:content-[''] before:w-28 sm:before:w-32 md:before:w-44 lg:before:w-48 xl:before:w-72 3xl:before:w-80 before:border-b before:absolute before:top-1/2 before:border-purple-700 before:right-full"
      >
